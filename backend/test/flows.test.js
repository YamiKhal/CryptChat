...
describe("login", () => {
    ...
    test("rejects the wrong password with an indistinguishable error", async () => {
        ...
        const noSuchUser = await new TestUser(uniqueName())
            .login("any-password-here")
            .catch((e) => e);
...
describe("password reset", () => {
    ...
    test("an unverified address cannot be used to reset", async () => {
        ...
        await assert.rejects(() => waitForMailLink("reset-password", { timeout: 1500, from }));
    });
});
...
describe("billing", () => {
    ...
    test("rejects a bogus redemption code", async () => {
        ...
        await user.register();
        await assert.rejects(
            () => user.redeem("AAAAA-BBBBB-CCCCC-DDDDD"),
            (e) => e.status === 400,
        );
    });
