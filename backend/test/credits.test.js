...
describe("gift credit", () => {
    ...
    test("the clock starts at redemption, not at purchase", async () => {
        ...
        assert.ok(
            months > 11.8,
            `a year-old gift must still grant 12 months, got ${months.toFixed(2)}`,
        );
    });
    ...
    test("parked credit activates once the subscription lapses", async () => {
        ...
        assert.ok(
            months > 2.8 && months < 3.2,
            `expected ~3 months from now, got ${months.toFixed(2)}`,
        );
...
describe("subscription entitlements", () => {
    ...
    test("a subscription keeps the expiry Stripe set", async () => {
        ...
        assert.ok(
            months > 2.8 && months < 3.2,
            `expected ~90 days, got ${months.toFixed(2)} months`,
        );
    });
});
