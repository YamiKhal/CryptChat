...
export function useComposer({
    ...
    const buildPreview = useCallback(
        async (body: string): Promise<LinkPreview | undefined> => {
            ...
            const url = pickPreviewUrl(
                body,
                vault.preferences.alwaysPreviewLinks,
            );
            ...
            try {
                ...
                if (meta.image) {
                    ...
                    if (
                        meta.kind === "image" &&
                        sniffed &&
                        raw.length <= MAX_INLINE_PREVIEW_BYTES
                    ) {
    ...
    const handleSend = useCallback(async () => {
        ...
        if (editingId) {
            ...
            if (overCharLimit(text, limits)) {
                setError(
                    `Message is over the ${limits.maxChars.toLocaleString()} character limit.`,
                );
                return;
            }
            setError("");
        ...
        if (lockArmed && !lockCode.trim()) {
            setError(
                "Enter a code to lock this message, or turn the lock off.",
            );
            return;
        }
    ...
    const handleUnlock = useCallback(
        async (message: StoredMessage, code: string) => {
            ...
            const updated = await vault.unlockMessage(
                channelId,
                message.id,
                code,
            );
    ...
    const jumpToMessage = useCallback((messageId: string) => {
        ...
        setTimeout(
            () => setHighlighted((c) => (c === messageId ? null : c)),
            1600,
        );
    }, []);
    ...
    const handleToggleReaction = useCallback(
        async (target: StoredMessage, emoji: string) => {
            if (!channelId) return;
            const mine =
                target.reactions?.[emoji]?.includes(account.userId) ?? false;
            try {
    ...
    async function handleFile(file: File | undefined) {
        ...
        if (!limits.canUpload) {
            setError(
                limits.uploadDenialReason ??
                    "Uploads are unavailable on this account.",
            );
            return;
        }
