...
export function buildMessageMenuItems(
  ...
  const knownSender = isOther && !ctx.incognito && Boolean(ctx.contacts[message.senderId]);
  ...
  const items: MenuItem[] = [
    ...
    },
    {
      ...
      icon: <Smile size={13} />,
      onSelect: () =>
        ctx.setReactingTo({ id: message.id, x: ctx.menuPos.x, y: ctx.menuPos.y }),
    },
  ];
  ...
  for (const attachment of message.attachments ?? []) {
    items.push({
      ...
      onSelect: () =>
        downloadAttachment(attachment, ctx.token!).catch((e) => ctx.setError(e.message)),
    });
  }
  ...
  if (knownSender) {
    items.push({
      label: ctx.isVerified(message.senderId) ? 'Safety number ✓' : 'Verify safety number',
      icon: <ShieldCheck size={13} />,
      onSelect: () => ctx.setVerifyingContact(ctx.contacts[message.senderId]),
    });
...
export function buildHeaderMenuItems(
  ...
  if (ctx.isDm && channel.peerId && !channel.incognito && ctx.contacts[channel.peerId]) {
    items.push({
      label: 'View profile',
      icon: <User size={14} />,
