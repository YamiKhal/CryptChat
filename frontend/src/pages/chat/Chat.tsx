...
export default function Chat() {
    ...
    const channel =
        channelId && vault ? vault.getChannel(channelId) : undefined;
    ...
    const [verifyingContact, setVerifyingContact] = useState<Contact | null>(
        null,
    );
    ...
    const { messages, setMessages, loading, limits, presenceLog, bottomRef } =
        useChannelMessages({
            vault,
            channelId,
            channel,
    ...
    const nameFor = useMemo(
        () => (userId: string) => {
            ...
            if (channel?.incognito && channelId)
                return incognitoLabel(channelId, userId);
            if (userId === account?.userId)
                return vault?.profile.displayName ?? "you";
    ...
    const typers = channelId
        ? typingIn(channelId).filter((id) => id !== account.userId)
        : [];
    ...
    return (
        <div className="flex h-full min-h-0 flex-col">
            ...
            {!channel.hasKey && (
                <div className="border-warn-line bg-warn-soft t-base text-warn border-b px-4 py-3">
                    <p className="font-medium">Waiting for the channel key</p>
                    <p className="text-warn mt-1">
                        An online member will pass it to you automatically. the
                        server never holds it. Until then, messages stay queued.
                    </p>
                </div>
            )}
            ...
            {composer.error && (
                <p className="border-error-line bg-error-soft t-base text-error border-t px-4 py-2">
                    {composer.error}
                </p>
            )}
            ...
            <ComposerAccessories
                ...
                onRemovePending={(blobId) =>
                    composer.setPending((c) =>
                        c.filter((a) => a.blobId !== blobId),
                    )
                }
            ...
            {composer.replyTo && (
                <ReplyComposing
                    reply={composer.replyTo}
                    onCancel={() => composer.setReplyTo(null)}
                />
            )}
          ...
          relay stores something it cannot read or scan. */}
            ...
            {actions.dmBlocked && (
                <div className="border-error-line bg-error-soft t-small text-error flex items-center justify-between border-t px-4 py-2">
                    <span>
                        You blocked this person. Their messages won't reach you.
                    </span>
                    <button
            ...
            <Composer
                ...
                canSend={
                    Boolean(composer.text.trim()) || composer.pending.length > 0
                }
                limits={limits}
                placeholder={
                    composer.editingId
            ...
            {actions.headerMenu && (
                <ContextMenu
                    ...
                    position={{
                        x: actions.headerMenu.x,
                        y: actions.headerMenu.y,
                    }}
                ...
                <QuickReactions
                    ...
                    onPick={(emoji) => {
                        const target = messages.find(
                            (message) => message.id === reactingTo.id,
                        );
                        if (target)
                            composer.handleToggleReaction(target, emoji);
                        setReactingTo(null);
                    }}
