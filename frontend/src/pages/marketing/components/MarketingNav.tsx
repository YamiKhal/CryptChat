...
function PillLink({ to, label }: { to: string; label: string }) {
    return (
        <NavLink
            to={to}
            className={({ isActive }) =>
                `t-base rounded-full px-4 py-1.5 font-medium transition-colors ${
                    isActive
...
export default function MarketingNav() {
    ...
    return (
        <header
            className={`border-border bg-surface sticky top-0 z-20 border-b transition-shadow duration-300 ${
                scrolled ? "shadow-lg" : ""
            }`}
        >
            <nav className="mx-auto flex h-16 max-w-6xl items-center gap-4 px-4 sm:px-6">
                <Link
                    to="/"
                    className="text-primary flex items-center gap-2 font-bold tracking-tight transition-transform hover:scale-[1.03]"
                >
            ...
            {/* Small screens: links drop under the bar so they never crowd the
                Launch App button. */}
            <div className="border-border flex items-center gap-1 border-t px-4 py-2 sm:hidden">
                {links.map((l) => (
                    <PillLink key={l.to} {...l} />
                ))}
            </div>
