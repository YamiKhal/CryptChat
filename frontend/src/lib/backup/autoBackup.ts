...
export async function configureBackup(
    ...
    if (!supportsFileSystemAccess()) {
        throw new AutoBackupError(
            "unsupported",
            "this browser cannot auto-backup to a file",
        );
    }
...
export async function reconnectBackup(userId: string): Promise<boolean> {
    ...
    if (!handle)
        throw new AutoBackupError("not-configured", "no backup file is set up");
    const granted =
...
export async function writeBackup(userId: string): Promise<void> {
    ...
    if (permission !== "granted") {
        throw new AutoBackupError(
            "permission",
            "backup file needs permission again",
        );
    }
