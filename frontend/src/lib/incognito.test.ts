 ...
 *
 * The property that matters for the feature's promise: a member's color and tag
 * are STABLE within a channel (so people can tell each other apart) but change
 * across channels (so nobody can use the color to link you). Exact values are
    ...
    it("spreads members across many hues, not all the same", () => {
        const hues = new Set(
            Array.from({ length: 50 }, (_, i) =>
                incognitoHue(CHAN_A, `sender-${i}`),
            ),
        );
        expect(hues.size).toBeGreaterThan(5);
    });

    it("gives the same sender different colors across channels", () => {
        // A single pair could collide by chance; across many channels it must not
        // be the same color every time -- that would mean the channel is ignored.
        const hues = new Set(
            Array.from({ length: 30 }, (_, i) =>
                incognitoHue(`channel-${i}`, SENDER),
            ),
        );
...
describe("incognitoLabel", () => {
    it("is stable for the same channel and sender", () => {
        expect(incognitoLabel(CHAN_A, SENDER)).toBe(
            incognitoLabel(CHAN_A, SENDER),
        );
    });
    ...
    it("differs across channels for the same sender (usually)", () => {
        const labels = new Set(
            Array.from({ length: 30 }, (_, i) =>
                incognitoLabel(`channel-${i}`, SENDER),
            ),
        );
    ...
    it("distinguishes different senders in one channel", () => {
        const labels = new Set(
            Array.from({ length: 50 }, (_, i) =>
                incognitoLabel(CHAN_A, `sender-${i}`),
            ),
        );
