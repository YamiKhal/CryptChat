...
export function applyCustomThemeVars(
    ...
    const primaryRgb =
        colors?.primary && isHexColor(colors.primary)
            ? toRgb(colors.primary)
            : null;
    if (primaryRgb) {
    ...
    const secondaryRgb =
        colors?.secondary && isHexColor(colors.secondary)
            ? toRgb(colors.secondary)
            : null;
    if (secondaryRgb) {
        root.style.setProperty(
    ...
    for (const bubbleVar of BUBBLE_VARS) {
        ...
        if (color && isHexColor(color))
            root.style.setProperty(bubbleVar.var, color);
