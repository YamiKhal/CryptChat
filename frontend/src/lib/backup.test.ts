...
import { Vault, saveAccount, AccountDescriptor, StoredMessage } from '@/lib/vault';
import { getSealed, migrateLocalStorageToIndexedDb } from '@/lib/vault/storage';
import { resetDbForTests } from '@/lib/vault/db';
import { buildBackup, restoreBackup, isBackupContainer } from '@/lib/backup/container';
...
async function seedDevice(): Promise<AccountDescriptor> {
  ...
  const vault = await Vault.create(USER, PASSWORD, {
    ...
    channels: {},
    contacts: {},
    profile: { displayName: 'Alice', updatedAt: '2026-01-01T00:00:00.000Z' },
  });
...
describe('full-vault backup', () => {
  ...
  it('restored vault stays sealed under the original password', async () => {
    ...
    await expect(Vault.unlock(USER, 'the-wrong-password')).rejects.toThrow(/wrong password/);
  });
  ...
  it('importBackup refuses a file belonging to another identity', async () => {
    await seedDevice();
    const container = await buildBackup(USER);
    await expect(importBackup(container!, 'a-different-user')).rejects.toThrow(
      /different identity/
    );
  });
  ...
  it('rejects structurally invalid files', () => {
    ...
    expect(isBackupContainer({ format: 'darkchat-backup', v: 1 })).toBe(false);
  });
});
...
describe('localStorage -> IndexedDB migration', () => {
  ...
  it('moves sealed blobs and leaves the plaintext registry alone', async () => {
    ...
    localStorage.setItem(`darkchat:msgs:${USER}:${CHANNEL}`, JSON.stringify(sealed));
    localStorage.setItem(`darkchat:acct:${USER}`, JSON.stringify({ userId: USER }));
    ...
    expect(localStorage.getItem(`darkchat:msgs:${USER}:${CHANNEL}`)).toBeNull();
    expect(await getSealed(`darkchat:vault:${USER}`)).toEqual(sealed);
    expect(await getSealed(`darkchat:msgs:${USER}:${CHANNEL}`)).toEqual(sealed);
