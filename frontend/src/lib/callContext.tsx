...
export function CallProvider({ children }: { children: ReactNode }) {
    ...
    const startCall = useCallback(
        async (cid: string, pid: string, kind: "audio" | "video") => {
            ...
            try {
                const { outgoingAudio, videoTrack } = await acquireLocal(
                    kind === "video",
                );
    ...
    const accept = useCallback(async () => {
        ...
        try {
            ...
            void sendSignal(call.channelId, {
                kind: "answer",
                callId: call.callId,
                sdp: answer.sdp,
            });
            ...
            if (channelIdRef.current)
                void sendSignal(channelIdRef.current, {
                    kind: "hangup",
                    callId: call.callId,
                });
            teardown();
    ...
    const decline = useCallback(() => {
        stopRingtone();
        if (incoming) {
            void sendSignal(incoming.channelId, {
                kind: "decline",
    ...
    const toggleScreenShare = useCallback(async () => {
        ...
        if (sharingScreen) {
            ...
            await videoSender
                .replaceTrack(cameraTrackRef.current ?? null)
                .catch(() => {});
            setSharingScreen(false);
