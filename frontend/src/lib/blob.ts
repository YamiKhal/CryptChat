...
const imageCache = new Map<string, { url: string; size: number; mime: string }>();
let cacheBytes = 0;
...
const THUMBNAILABLE = new Set(['image/png', 'image/jpeg', 'image/webp', 'image/gif', 'image/avif']);
...
export async function encryptAndUpload(
  ...
  for (let index = 0; index < chunkCount; index++) {
    ...
    onProgress?.({ loaded: uploaded, total: file.size, phase: 'uploading' });
  }
  ...
  if (THUMBNAILABLE.has(file.type)) {
    try {
      ...
      thumb = packAsset(
        await encodeImage(file, { maxDimension: THUMB_MAX_DIMENSION, mime: 'image/webp', quality: 0.7 })
      );
    } catch {
      thumb = undefined;
    }
...
export async function downloadAndDecrypt(
  ...
  const decryptor = await createFileDecryptor(attachment.key, attachment.header);
  const hasher = await createHasher();
  ...
  const consume = (chunk: Bytes) => {
    ...
    onProgress?.({ loaded: plaintextBytes, total: attachment.size, phase: 'downloading' });
  };
  ...
  onProgress?.({ loaded: attachment.size, total: attachment.size, phase: 'done' });
  ...
  return new Blob(parts as unknown as BlobPart[], { type: 'application/octet-stream' });
}
