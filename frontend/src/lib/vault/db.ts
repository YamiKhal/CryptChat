...
export function dbGet<T>(key: string): Promise<T | null> {
    return run<T | undefined>("readonly", (s) => s.get(key)).then(
        (v) => v ?? null,
    );
}

export function dbPut(key: string, value: unknown): Promise<void> {
    return run<IDBValidKey>("readwrite", (s) => s.put(value, key)).then(
        () => undefined,
    );
}

export function dbDelete(key: string): Promise<void> {
    return run<undefined>("readwrite", (s) => s.delete(key)).then(
        () => undefined,
    );
}
