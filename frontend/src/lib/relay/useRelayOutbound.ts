...
export function useRelayOutbound(
    ...
    const send = useCallback(
        async (
            ...
            const supporter =
                !channel.incognito && v.preferences.showSupporterBadge
                    ? true
                    : undefined;
    ...
    const sendReaction = useCallback(
        async (
            ...
            if (!isSingleEmoji(emoji))
                throw new Error("reactions must be a single emoji");
            ...
            await v.applyReactionToMessage(
                channelId,
                targetId,
                emoji,
                userId,
    ...
    const sendSignal = useCallback(
        async (channelId: string, signal: CallSignal) => {
            const v = vaultRef.current;
            const ws = wsRef.current;
            if (!v || !userId || !ws || ws.readyState !== WebSocket.OPEN)
                return;
