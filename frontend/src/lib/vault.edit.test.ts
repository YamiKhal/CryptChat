...
import { generateIdentity, sealWithPassword } from './crypto';
...
describe('deleteMessage', () => {
  ...
  it('lets the author delete, leaving a tombstone with no plaintext', async () => {
    const vault = await vaultWith({ ...msg('m1', SELF), body: 'secret words' });
...
describe('unlockMessage (ROADMAP #6)', () => {
  ...
  async function lockedMsg(id: string, plaintext: string, code: string): Promise<StoredMessage> {
  ...
  it('reveals the body with the right code and clears the lock', async () => {
    const vault = await vaultWith(await lockedMsg('m1', 'the eagle lands at noon', 'owl'));
    ...
    expect((await bodyOf(vault, 'm1'))?.body).toBe('the eagle lands at noon');
  });

  it('a wrong code throws and leaves the message locked', async () => {
    const vault = await vaultWith(await lockedMsg('m1', 'secret', 'owl'));
    await expect(vault.unlockMessage(CHAN, 'm1', 'cat')).rejects.toThrow(/wrong code/);
