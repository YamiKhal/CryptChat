...
export function handleCallSignal(
  ...
  { channelId: cid, senderId, signal }: { channelId: string; senderId: string; signal: CallSignal },
) {
  switch (signal.kind) {
    case 'offer': {
      // Busy: reject a second caller rather than clobber the live call.
      if (ctx.status !== 'idle' || ctx.pcRef.current) {
        void ctx.sendSignal(cid, { kind: 'decline', callId: signal.callId });
        return;
      }
      ...
      if (signal.media === 'video' && !ctx.premium) {
        void ctx.sendSignal(cid, { kind: 'decline', callId: signal.callId });
        ctx.setError('A video call needs a supporter account on both sides.');
        return;
      }
    ...
    case 'answer': {
      if (signal.callId !== ctx.callIdRef.current || !ctx.pcRef.current || !signal.sdp) return;
      ...
      void ctx.pcRef.current
        ...
        .then(async () => {
          for (const c of ctx.pendingIceRef.current) await ctx.pcRef.current!.addIceCandidate(c).catch(() => {});
          ctx.pendingIceRef.current = [];
        })
        .catch(() => {});
      break;
    ...
    case 'ice': {
      if (signal.callId !== ctx.callIdRef.current && signal.callId !== ctx.pendingOfferRef.current?.callId) return;
    ...
    case 'video': {
      ...
      if (signal.callId !== ctx.callIdRef.current && signal.callId !== ctx.pendingOfferRef.current?.callId) {
        return;
      }
