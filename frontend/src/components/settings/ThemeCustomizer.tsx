...
export default function ThemeCustomizer({ vault, isPremium, onChange }: Props) {
    ...
    const [colors, setColors] = useState<Record<string, string>>(
        prefs.customTheme?.colors ?? {},
    );
    const [bubbles, setBubbles] = useState<BubbleTheme>(
        prefs.customTheme?.bubbles ?? {},
    );
    ...
    async function chooseBackground(e: ChangeEvent<HTMLInputElement>) {
        ...
        try {
            ...
            const animated =
                file.type === "image/gif" || file.type.startsWith("video/");
            let asset: BinaryAsset;
            if (animated) {
                if (file.size > 8 * 1024 * 1024) {
                    setError(
                        "An animated or video wallpaper must be under 8MB.",
                    );
                    return;
                }
    ...
    if (!isPremium) {
        return (
            <SettingsSection title="Custom theme">
                <SettingBlock>
                    <p className="t-base text-warn flex items-center gap-1.5">
                        <Crown
                            size={13}
                ...
                <SettingBlock>
                    <div className="grid grid-cols-2 gap-2">
                        {CUSTOMIZABLE_TOKENS.map((token) => (
                            <label
                                key={token}
                                className="border-border bg-surface-raised flex items-center gap-2 rounded border px-2 py-1.5"
                            >
                                <input
                                    type="color"
                    ...
                    <div className="space-y-2">
                        <p className="t-small text-muted font-medium">
                            Message bubbles
                        </p>
                        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                            ...
                            <BubbleControl
                                ...
                                color={
                                    bubbles.otherBg ??
                                    seedColor(colors, "surface-raised")
                                }
            ...
            <SettingBlock>
                ...
                {bgUrl &&
                    (background?.mime.startsWith("video/") ? (
                        <video
                            src={bgUrl}
                            className="border-border h-20 w-full rounded border object-cover"
                            autoPlay
                            loop
                            muted
                            playsInline
                        ...
                        <div
                            className="border-border h-20 w-full rounded border bg-cover bg-center"
                ...
                <div className="flex gap-2">
                    ...
                    {bgUrl && (
                        <button
                            ...
                            className="btn-ghost t-base text-error"
                        >
                            remove
                        </button>
                    )}
...
function BubbleControl({
    ...
    return (
        <div className="border-border bg-surface-raised flex items-center justify-between gap-2 rounded border p-2">
