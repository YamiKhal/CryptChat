        ...
        >
            <div className="space-y-1">
                <div className="flex items-center gap-1.5">
                    <h2
                        className={`t-base font-semibold tracking-wider uppercase ${
                ...
                {description && (
                    <p className="t-base text-muted leading-snug">
                        {description}
                    </p>
                )}
            </div>
    ...
    return (
        <div className="flex items-start justify-between gap-4 py-3.5">
            <div className="min-w-0 flex-1 space-y-1.5">
                <div className="flex items-center gap-1.5">
                    <span className="t-h4 text-foreground font-medium">
                        {title}
                    </span>
                    {info && (
                        <InfoTip
...
export function SegmentedControl<T extends string>({
    ...
    return (
        <div className="border-border bg-surface-raised inline-flex rounded-lg border p-0.5">
            {options.map((o) => (
                <button
                    key={o.value}
                    onClick={() => onChange(o.value)}
                    aria-pressed={value === o.value}
                    className={`t-base rounded-md px-2.5 py-1 transition-colors motion-reduce:transition-none ${
                        value === o.value
