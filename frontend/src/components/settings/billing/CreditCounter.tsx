 ...
 */
export default function CreditCounter({ creditMonths = 0 }: { creditMonths?: number }) {
    ...
    <div className="flex items-center gap-3 rounded-lg border border-border bg-surface-raised p-3">
      ...
      <div className="min-w-0">
        <div className="flex items-baseline gap-1.5">
          <span className={`t-h2 font-semibold tabular-nums ${has ? 'text-info' : 'text-muted'}`}>
            {creditMonths}
          </span>
          <span className="t-base text-muted">gifted {unit} banked</span>
        </div>
