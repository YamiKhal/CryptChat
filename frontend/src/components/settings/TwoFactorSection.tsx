...
export default function TwoFactorSection({ token }: { token: string }) {
  ...
  async function enroll() {
    ...
    setBusy(true);
    try {
      const { options, challengeToken } = await api.twoFactorRegisterOptions(token);
      ...
      setLabel('');
      setStatus({ kind: 'ok', text: 'Security key added. It is now required at login.' });
      reload();
    } catch (err) {
      // A user cancelling the browser prompt lands here too; keep it gentle.
      setStatus({ kind: 'error', text: (err as Error).message || 'enrollment cancelled' });
    } finally {
      setBusy(false);
    }
  }
