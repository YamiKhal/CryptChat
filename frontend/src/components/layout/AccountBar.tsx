...
export default function AccountBar() {
    ...
    return (
        <>
            <div className="border-border bg-surface flex min-h-16 shrink-0 items-center gap-2 border-t px-3">
                {/* Left: identity. Click opens your card; right-click / long-press opens
            the account menu. Only this half is the profile handle. */}
                <button
                    type="button"
                    {...menu.handlers}
                    className="hover:bg-surface-raised -ml-1 flex min-w-0 flex-1 items-center gap-3 rounded-lg px-1.5 py-1 text-left transition-colors"
            ...
            up to mark it active and a second click returns to the channels. */}
                <button
                    onClick={() =>
                        navigate(onSettings ? "/channels" : "/settings")
                    }
