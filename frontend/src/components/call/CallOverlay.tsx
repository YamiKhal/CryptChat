      ...
      <div className="fixed inset-x-0 top-4 z-[60] flex justify-center px-4">
        <div className="menu-panel w-full max-w-sm p-4">
          <div className="flex items-center gap-3">
            <Avatar asset={avatarFor(inc.peerId)} name={nameFor(inc.peerId)} size="md" />
            <div className="min-w-0 flex-1">
              <p className="truncate t-h4 font-semibold">{nameFor(inc.peerId)}</p>
              <p className="flex items-center gap-1.5 t-small text-muted">
                {inc.media === 'video' ? <Video size={12} /> : <Phone size={12} />}
                incoming {inc.media === 'video' ? 'video call' : 'call'}
              </p>
            </div>
            <div className="flex flex-none items-center gap-2.5">
              <button
                ...
                aria-label="Decline"
                title="Decline"
                className="grid size-11 place-items-center rounded-full bg-error text-white
              ...
              <button
                ...
                aria-label="Accept"
                title="Accept"
                className="grid size-11 place-items-center rounded-full bg-ok text-white
                           transition-transform hover:scale-105"
              >
                <Phone size={18} />
              </button>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-black">
      ...
      <div className="flex items-center gap-3 px-4 py-3 text-white sm:px-6">
        <div className="min-w-0 flex-1">
          <p className="truncate t-h4 font-semibold">{nameFor(call.peerId)}</p>
          <p className="t-small text-zinc-400">
            {ringing ? 'calling…' : connected ? <CallTimer /> : 'connecting…'}
          </p>
        </div>
        {!call.relayAvailable && !connected && (
          <span className="rounded-full bg-zinc-800 px-2.5 py-1 t-small text-zinc-300">
        ...
        {showStage ? (
          <>
            ...
            {call.remoteHasVideo && iSendVideo && (
              <VideoTile
                ...
                className="absolute right-3 bottom-3 h-20 w-32 rounded-xl border border-zinc-700
                           object-cover shadow-lg sm:right-5 sm:bottom-5 sm:h-28 sm:w-44"
                muted
              />
            )}
          </>
          ...
          <div className="grid h-full place-items-center">
            <div className="flex flex-col items-center gap-4">
              ...
              <p className="t-h3 font-semibold text-white">{nameFor(call.peerId)}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-center gap-3 px-4 pt-4 pb-8 sm:gap-4 sm:pb-6">
        ...
        {call.premium && (
          <ControlButton
            ...
            label={call.sharingScreen ? 'Stop sharing' : 'Share screen'}
          >
            <MonitorUp size={20} />
          </ControlButton>
        )}
        ...
        <button
          onClick={call.hangup}
          className="grid h-13 w-17 place-items-center rounded-full bg-error text-white
                     transition-transform hover:scale-105"
          aria-label="End call"
          title="End call"
        >
