...
export function ChannelIcon({
  ...
  if (channel.type === 'dm') {
    return <Avatar asset={peerAvatar} name={peerName || 'direct message'} size={size} />;
  }

  if (channel.icon) {
    return <Avatar asset={channel.icon} name={channel.label || 'Group'} size={size} />;
  }

  return (
    <div
      className={`${BOX[size]} grid shrink-0 place-items-center rounded-full border border-border
