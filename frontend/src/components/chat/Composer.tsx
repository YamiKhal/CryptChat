...
export default function Composer({
    ...
    return (
        ...
        <div className="border-border bg-surface relative flex min-h-16 flex-col justify-center border-t px-3 py-2">
            {showEmoji && (
                <EmojiPicker
                    onPick={(emoji) => {
                        insertEmoji(emoji);
            ...
            while typing; over-limit turns it red. */}
                <div
                    className={`bg-surface-raised flex min-h-11 min-w-0 flex-1 items-center gap-0.5 rounded-2xl border px-1.5 transition-colors ${
                        over
                            ? "border-error"
                ...
                >
                    ...
                    <textarea
                        ...
                        className="t-h4 placeholder:text-muted max-h-[140px] min-w-0 flex-1 resize-none overflow-y-hidden bg-transparent px-1.5 py-2.5 leading-snug wrap-break-word outline-none"
                    />
                        ...
                        <div className="relative flex-none">
                            {showTools && (
                                <>
                                    ...
                                    />
                                    <div className="menu-panel absolute right-0 bottom-full z-50 mb-2 w-52 origin-bottom">
                                        {canLock && (
                                            <button
                                                onClick={() => {
                                                    onToggleLock?.();
            ...
            {showCounter && (
                <p
                    className={`t-small mt-1 text-right tabular-nums ${over ? "text-error" : "text-muted"}`}
                >
                    ...
                    {over && !limits.premium && (
                        <span className="ml-1">
                            — supporters get {(4000).toLocaleString()}
                        </span>
                    )}
                </p>
            )}
