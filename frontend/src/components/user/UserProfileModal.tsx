...
function renderBio(bio: string): ReactNode {
    ...
    for (const match of bio.matchAll(BIO_LINK)) {
        ...
        if (start > cursor)
            out.push(<Fragment key={i++}>{bio.slice(cursor, start)}</Fragment>);
        out.push(
            <a
                key={i++}
                href={url}
    ...
    if (cursor < bio.length)
        out.push(<Fragment key={i++}>{bio.slice(cursor)}</Fragment>);
    return out;
}
...
function Banner({ profile }: { profile: UserProfile }) {
    ...
    return (
        <div className="bg-primary-soft h-28 w-full">
            {url && (
        ...
        <div className="modal-backdrop" onClick={onClose}>
            ...
            >
                ...
                <button
                    onClick={onClose}
                    aria-label="Close"
                    className="bg-surface text-muted hover:text-foreground absolute top-2.5 right-2.5 z-10 grid size-8 place-items-center rounded-full transition-colors"
                >
                    <X size={16} />
                </button>
                ...
                <div className="space-y-2 px-4 pt-0 pb-4">
                    <div className="ring-surface -mt-10 w-fit rounded-full ring-4">
                        <Avatar
                    ...
                    <p className="t-h3 text-foreground font-bold">
                        {profile.displayName}
                    </p>

                    {profile.bio && (
                        <div className="bg-surface-raised rounded-lg p-3">
                            <p className="t-base text-foreground wrap-break-word whitespace-pre-wrap">
