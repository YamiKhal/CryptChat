...
described itself _exactly_ as truthfully as it behaved? Not "we can't" when the
...
genuinely hard consequence: if the server can't help you recover, _how do you
ever get back in?_
...
anywhere in the universe". anyone holding _both_ sides could join them on the
...
A message means nothing if two devices can't agree on _which_ message it is.
...
bookkeeping. Distributed identity is deceptively easy to get _almost_ right and
...
same key, so decryption alone proves nothing about _who_ wrote a message. Each
...
Replies and reactions live _inside_ that signed envelope, so a relay can't
...
The honesty extends to what the relay _does_ unavoidably see: which user ids
...
- **Link previews built by the sender.** Off by default; when asked, _your_
...
supporter feature and _both_ sides must be premium. Screen-share only gates the
...
config lives _in the encrypted vault_, syncing across a user's own devices
...
and a plain one would have to _decrypt_ the stream to route it. a hard no here.
...
the _interface_. no names, no avatars. but the envelope still carries a real
